...
object EpicCloudSavesManager {
        ...
        try {
            ...
            files.forEach { file ->
                try {
                    ...
                    val fileManifest = com.winlator.cmod.feature.stores.epic.service.manifest.FileManifest()
                    ...
                    while (fileOffset < fileData.size) {
                        ...
                        val chunkPart = com.winlator.cmod.feature.stores.epic.service.manifest.ChunkPart(
    ...
    private fun finalizeChunk(
        ...
        val chunkInfo = com.winlator.cmod.feature.stores.epic.service.manifest.ChunkInfo()
        chunkInfo.guid = guid
    ...
    private fun createManifest(
        ...
        val manifest = com.winlator.cmod.feature.stores.epic.service.manifest.BinaryManifest()

        // Meta
        manifest.meta = com.winlator.cmod.feature.stores.epic.service.manifest.ManifestMeta()
        ...
        // Custom fields
        manifest.customFields = com.winlator.cmod.feature.stores.epic.service.manifest.CustomFields()
        manifest.customFields!!["CloudSaveFolder"] = game.saveFolder

        // Chunks
        manifest.chunkDataList = com.winlator.cmod.feature.stores.epic.service.manifest.ChunkDataList()
        manifest.chunkDataList!!.elements.addAll(chunks)

        // Files
        manifest.fileManifestList = com.winlator.cmod.feature.stores.epic.service.manifest.FileManifestList()
    ...
    private fun resolveSaveDirectory(context: Context, game: EpicGame, accountId: String): File? {
        ...
        val container = com.winlator.cmod.feature.stores.steam.utils.ContainerUtils.getUsableContainerOrNull(context, appId)
            ?: return null
